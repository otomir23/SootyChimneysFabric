...
object ModLootTables {
    ...
    fun getSootScrapingLootFor(state: BlockState, level: ServerLevel): List<ItemStack> {
        ...
        val blockId = BuiltInRegistries.BLOCK.getResourceKey(chimney.dirtyVariant).get().location().path
        return try {
            val lootContext = LootParams.Builder(level)
                .withParameter(LootContextParams.BLOCK_STATE, state)
                .create(SOOT_SCRAPING_PARAM_SET)
            val table = level.server.lootData.getLootTable(ResourceLocation("$SOOT_SCRAPING/$blockId"))
